...
/* ─── Full International Country Calling Codes ─────────────────────────── */
const COUNTRY_CODES = [
...
const formSchema = z.object({
    ...
    phone: z.string().min(1, "Phone number is required").regex(/^\d{10,}$/, "Min 10 digits only"),
    whatsappNumber: z.string().min(1, "WhatsApp number is required").regex(/^\d{10,}$/, "Min 10 digits only"),
...
/* ─── Shared styles ────────────────────────────────────────────────────── */
const inputCls = "w-full bg-white/[0.05] border border-white/10 rounded-xl px-3.5 py-2.5 text-white text-sm placeholder:text-slate-600 focus:outline-none focus:border-blue-500/50 focus:bg-white/[0.07] transition-all";
const labelCls = "block text-[11px] font-semibold text-slate-400 mb-1.5 uppercase tracking-wider";

function FieldError({ msg }: { msg?: string }) {
    if (!msg) return null;
    return <p className="text-red-400 text-[11px] mt-1 leading-tight">{msg}</p>;
}
    ...
    return (
        <div className="relative">
            <button
                ...
                disabled={disabled}
                onClick={openMenu}
                className={`flex items-center justify-between w-full bg-white/[0.05] border ${error ? "border-red-500/50" : "border-white/10"
                    ...
                    <div className="max-h-52 overflow-y-auto">
                        {filtered.length === 0 ? (
                            ...
                            <button
                                ...
                                className={`flex w-full px-3.5 py-2 text-sm hover:bg-white/[0.07] text-left transition-colors ${o.value === value ? "text-white bg-white/[0.06]" : "text-slate-300"
                                    }`}
                            >
                                {o.label}
                            </button>
...
function CountryCodePicker({ value, onChange, error }: { value: string; onChange: (v: string) => void; error?: string }) {
    ...
    const selected = COUNTRY_CODES.find(c => c.code === value && c.name !== "Canada") || COUNTRY_CODES.find(c => c.code === value) || COUNTRY_CODES[0];
    ...
    return (
        <div className="relative">
            <button
                ...
                type="button"
                onClick={openMenu}
                className={`flex items-center gap-1.5 bg-white/[0.05] border ${error ? "border-red-500/50" : "border-white/10"
            ...
            {open && rect && createPortal(
                ...
                >
                    <div className="p-2 border-b border-white/10">
                        <div className="flex items-center gap-2 bg-white/[0.05] rounded-lg px-2 py-1.5">
                    ...
                    <div className="max-h-52 overflow-y-auto">
                        {filtered.map((c, i) => (
                            <button
                                ...
                                className="flex items-center gap-2 w-full px-3 py-2 text-sm hover:bg-white/[0.07] text-left transition-colors"
                            >
                                <span>{c.flag}</span>
    ...
    return (
        <div>
            <div className={`flex items-start gap-3 p-3.5 rounded-xl border transition-colors ${checked ? accentClass : "bg-white/[0.03] border-white/10"}`}>
                {/* ── Checkbox square: ONLY this toggles the value ── */}
                <button
                    type="button"
                    onClick={onToggle}
                ...
                {/* ── Text area: no onClick — clicks here do NOT toggle ── */}
    ...
    return createPortal(
        <div className="fixed inset-0 flex items-end sm:items-center justify-center p-0 sm:p-4" style={{ zIndex: 10000 }}>
            ...
            >
                ...
                {/* Scrollable body — flex-1 + overflow-y-auto is the key */}
                ...
                {/* Footer — flex-shrink-0 so it never collapses */}
...
export function CopyTradingPopup({ externalTrigger, onExternalTriggerConsumed }: CopyTradingPopupProps) {
    ...
    const countryOptions = Country.getAllCountries().map(c => ({ value: c.isoCode, label: `${c.flag ?? ""} ${c.name}` }));
    ...
    const { register, handleSubmit, watch, setValue, formState: { errors, isSubmitting }, reset } = useForm<FormValues>({
        ...
        defaultValues: {
            ...
            riskAccepted: undefined as any,
            feeAccepted: undefined as any,
    ...
    const handleOpenChange = useCallback((val: boolean) => {
        setOpen(val);
        if (!val) setTimeout(() => { setStep("teaser"); reset(); }, 300);
    }, [reset]);

    const onSubmit = async (data: FormValues) => {
        const selectedCountryName = Country.getCountryByCode(data.countryIso)?.name || data.countryIso;
        const selectedStateName = State.getStateByCodeAndCountry(data.stateIso, data.countryIso)?.name || data.stateIso;
        const payload = {
            ...data, age: data.age ? Number(data.age) : null,
            country: selectedCountryName, state: selectedStateName,
            riskAccepted: true, feeAccepted: true, termsAccepted: true,
        };
    ...
    return (
        <>
            {/* Terms modal via portal — renders at document.body, fully outside all stacking contexts */}
            <AnimatePresence>
            ...
            <Dialog open={open} onOpenChange={handleOpenChange}>
                <DialogContent className="p-0 border-0 bg-transparent shadow-none max-w-xl w-full">
                    ...
                    <div className="relative w-full max-w-xl mx-auto rounded-2xl overflow-hidden bg-[#0b1623] border border-white/10 shadow-2xl shadow-blue-950/70">
                        ...
                        <AnimatePresence mode="wait" initial={false}>
                            ...
                            {step === "teaser" && (
                                <motion.div key="teaser" variants={fadeVariants} initial="enter" animate="center" exit="exit"
                                    ...
                                    className="relative z-10 flex flex-col items-center text-center px-8 py-12 sm:py-14"
                                >
                                    <div className="flex items-center gap-3 mb-8 text-[10px] font-bold uppercase tracking-widest text-slate-600">
                                    ...
                                    <div className="w-16 h-16 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center mb-5 shadow-lg shadow-emerald-900/30">
                                        <TrendingUp className="w-8 h-8 text-emerald-400" />
                                    </div>

                                    <h2 className="text-2xl sm:text-3xl font-black text-white leading-tight mb-3 max-w-xs sm:max-w-sm">
                                    ...
                                    <div className="flex items-center gap-2 mb-6 px-5 py-2.5 rounded-full bg-amber-500/10 border border-amber-500/25 text-amber-400 font-black text-lg">
                                    ...
                                    <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
                                        ...
                                        className="flex items-center gap-2.5 px-10 py-4 rounded-2xl font-black text-base text-white shadow-2xl shadow-emerald-900/40"
                                        style={{ background: "linear-gradient(135deg, #059669 0%, #0ea5e9 100%)" }}
                                    >
                                            ...
                                            <div className="space-y-4">
                                                ...
                                                <div className="grid grid-cols-3 gap-2">
                                                    <div>
                                                        ...
                                                        <input {...register("firstName")} type="text" placeholder="Rahul" className={inputCls} />
                                                        <FieldError msg={errors.firstName?.message} />
                                                    </div>
                                                    <div>
                                                        <label className={labelCls}>Middle <span className="text-slate-600 normal-case font-normal">(opt)</span></label>
                                                        <input {...register("middleName")} type="text" placeholder="Kumar" className={inputCls} />
                                                    </div>
                                                    <div>
                                                        <label className={labelCls}>Last *</label>
                                                        <input {...register("lastName")} type="text" placeholder="Sharma" className={inputCls} />
                                                ...
                                                {/* Email */}
                                                <div>
                                                    <label className={labelCls}><span className="flex items-center gap-1.5"><Mail className="w-3 h-3 inline" /> Email *</span></label>
                                                    <input {...register("email")} type="email" placeholder="yourname@gmail.com" className={inputCls} />
                                                ...
                                                <div>
                                                    ...
                                                    <div className="flex gap-2">
                                                        ...
                                                        <div className="flex-1">
                                                            <input {...register("phone")} type="tel" placeholder="9876543210" inputMode="numeric" className={inputCls}
                                                ...
                                                {/* WhatsApp */}
                                                <div>
                                                    <label className={labelCls}><span className="flex items-center gap-1.5"><MessageCircle className="w-3 h-3 inline" /> WhatsApp *</span></label>
                                                    <input {...register("whatsappNumber")} type="tel" placeholder="9876543210" inputMode="numeric" className={inputCls}
                                                        onKeyPress={e => { if (!/[0-9]/.test(e.key)) e.preventDefault(); }} />
                                                ...
                                                {/* Telegram */}
                                                <div>
                                                    <label className={labelCls}><span className="flex items-center gap-1.5"><Send className="w-3 h-3 inline" /> Telegram ID *</span></label>
                                                    <input {...register("telegramId")} type="text" placeholder="@yourusername" className={inputCls} />
                                                ...
                                                <div className="grid grid-cols-2 gap-3">
                                                    <div>
                                                        ...
                                                        <input {...register("age")} type="number" placeholder="18+" min={18} max={120} className={inputCls} />
                                                ...
                                                <div>
                                                    ...
                                                    <textarea {...register("address")} rows={2} placeholder="Street, Area..." className={`${inputCls} resize-none`} />
                                                ...
                                                <div className="space-y-3 pt-0.5">
                                                    <ConsentCheckbox
                                                        ...
                                                        onToggle={() => setValue("riskAccepted", riskAccepted ? (undefined as any) : true)}
                                                    ...
                                                    <ConsentCheckbox
                                                        checked={!!feeAccepted}
                                                        onToggle={() => setValue("feeAccepted", feeAccepted ? (undefined as any) : true)}
                                                    ...
                                                    <ConsentCheckbox
                                                        ...
                                                        onToggle={() => setValue("termsAccepted", termsAccepted ? (undefined as any) : true)}
